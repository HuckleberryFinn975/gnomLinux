...
lettersList = ("t", "v", "ya",  "l", "a", "c", "n", "z", "f", "j", "i", "m", "r", "b", "d", "g", "y", "k", "o", "sh4", "u", "'", "ch", "e", "h", "p", "sh", "u_", "y__", "e_", "s", "zh")

def recognizeLetters(input_file):
    image = cv2.imread(input_file, cv2.IMREAD_GRAYSCALE)
    image = cv2.threshold(image, 128, 255, cv2.THRESH_BINARY_INV)[1]

    digit_templates = {}
    template_folder = "Letters"
    for digit in lettersList:
        template_path = os.path.join(template_folder, f"{digit}.png")
